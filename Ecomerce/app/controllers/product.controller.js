...
module.exports = {
  add: function (req, res) {
    ...
    if (req.file.filename == null) {
      return res
        .status(400)
        .json({ success: false, error: 'missing parameter (file)' });
    }
  ...
  getByIdWithDetais: function (req, res) {
    ...
    Product.findOne({
      ...
      .catch(function (err) {
        return res
          .status(500)
          .json({ success: false, error: 'unable to get product' });
      });
  },
          ...
          productFound
            .update({
              ...
              prixPromotion: prixPromotion
                ? prixPromotion
                : productFound.prixPromotion,
              promoPercentage: promoPercentage
                ? promoPercentage
                : productFound.promoPercentage,
              isSponsor: isSponsor ? isSponsor : productFound.isSponsor,
            })
            .then(function (productFound) {
              return res.status(201).json({success : true,productFound,message:'success edit'});
            })
            .catch(function (err) {
              return res.status(500).json({ success : false,error: ' cant update product' });
            });
        } else {
          return res.status(404).json({success : false, error: 'Product not found in db' });
        }
      })
      .catch(function (err) {
        return res.status(500).json({ success : false,error: 'unable to verify product' });
      });
  },
  getById: function (req, res) {
    ...
    Product.findOne({
      ...
      .catch(function (err) {
        return res.status(500).json({ success : false,error: 'unable to get product' });
      });
  },
  ...
  getAllByIdCategory: function (req, res) {
    ...
    Product.findAll({
      ...
      .catch(function (err) {
        return res
          .status(500)
          .json({ success: false, message: 'unable to get product' });
      });
  },
};
